...
from diffusers.models.parametrizations import OFTModule
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--train_text_encoder",
        action="store_true",
        help="Whether to train OFT weights in the text encoder. If set, the text encoder should be float32 precision.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reduction_rate",
        type=int,
        help=("The number of blocks in the orthogonal matrix."),
    )
    parser.add_argument(
...
def main(args):
    ...
    oft_params_unet = []
    oft_layers_unet = ParametrizationsLoaderMixin.set_parametrizations_modules(
        unet,
        OFTModule,
        filter_module_name_fn=lambda s: "attn" in s,
        reduction_rate=args.reduction_rate,
        epsilon=args.epsilon,
        use_coft=args.use_coft,
        share_blocks=args.share_oft_blocks,
    )
    for module in oft_layers_unet.values():
        oft_params_unet.extend(module.parameters())

    if args.train_text_encoder:
        text_encoder.to(accelerator.device)
        oft_params_text_encoder = []
        oft_layers_text_encoder = ParametrizationsLoaderMixin.set_parametrizations_modules(
            text_encoder,
            OFTModule,
            filter_module_name_fn=lambda s: "attn" in s,
            reduction_rate=args.reduction_rate,
        ...
        for module in oft_layers_text_encoder.values():
            oft_params_text_encoder.extend(module.parameters())
    else:
    ...
    if args.gradient_checkpointing:
        ...
        if args.train_text_encoder:
            # Enable gradients for token embed to ensure gradient flow to OFT weights when gradient checkpointing
            # For more information, see discussion at https://github.com/kohya-ss/sd-scripts/issues/323
    ...
    def load_model_hook(models, input_dir):
        ...
        coft_args = {"epsilon": args.epsilon, "use_coft": args.use_coft}
        parametrizations_state_dict, parametrizations_class = ParametrizationsLoaderMixin.parametrizations_state_dict(
            input_dir
        )
        ParametrizationsLoaderMixin.load_parametrizations_into_unet(
            parametrizations_state_dict, parametrizations_class, unet_, **coft_args
        )
        ParametrizationsLoaderMixin.load_parametrizations_into_text_encoder(
            parametrizations_state_dict, parametrizations_class, text_encoder_, **coft_args
        )
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(oft_params_unet, oft_params_text_encoder) if args.train_text_encoder else oft_params_unet
    )
    ...
    # TODO: Calculate the hyperspherical energy before fine-tuning
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(oft_params_unet, oft_params_text_encoder)
                        if args.train_text_encoder
                        else oft_params_unet
                    )
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                # TODO: Calculate the hyperspherical energy after fine-tuning
    ...
    if accelerator.is_main_process:
        ...
        for module_key, param_module in ParametrizationsLoaderMixin.get_parametrizations_modules(
            ...
            for parameter_key, parameter in param_module.state_dict().items():
                unet_oft_layers[f"{module_key}.{parameter_key}"] = parameter

        if text_encoder is not None and args.train_text_encoder:
            text_encoder = accelerator.unwrap_model(text_encoder)
            text_encoder = text_encoder.to(torch.float32)

            text_encoder_oft_layers = {}
            for module_key, param_module in ParametrizationsLoaderMixin.get_parametrizations_modules(
                text_encoder, OFTModule
            ).items():
                for parameter_key, parameter in param_module.state_dict().items():
                    text_encoder_oft_layers[f"{module_key}.{parameter_key}"] = parameter
        else:
            text_encoder_oft_layers = None

        ParametrizationsLoaderMixin.save_parametrizations_weights(
            save_directory=args.output_dir,
            unet_parametrizations_layers=unet_oft_layers,
            text_encoder_parametrizations_layers=text_encoder_oft_layers,
        )
        ...
        # load layer parametrization weights after moving to accelerator device to guard against issues in fp16
        pipeline = pipeline.to(accelerator.device)
        pipeline.load_parametrizations_weights(args.output_dir)
